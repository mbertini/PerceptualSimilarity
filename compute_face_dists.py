...
from MSSIM import compute_MSSISM
import numpy as np
import fnmatch
import os
import models
...
def main():
    ...
    ap.add_argument('-m0', '--mask0', required=True, type=str, default='.mp4',
    ...
    ap.add_argument('-m1', '--mask1', required=True, type=str, default='.mkv',
                    help='file mask for files in d1 (e.g. .mp4)')
    ap.add_argument('-o', '--out', required=True, type=str, default='./imgs/example_dists.txt',
                    help='Distance file (txt)')
    ...
    fd = FaceDetector(verbose=args.verbose)
    ...
    for reference_video_name, modified_video_name in zip(dir0_files, dir1_files):
        ...
        out_file_MSSIM.writelines('frame_num, MSSIM distance\n')
        ...
        modified_video = cv2.VideoCapture(modified_video_name)
        frame_count = 0
        while reference_video.isOpened() and modified_video.isOpened():
            ...
            if not ref_ret_val:
                print("Can not access video: " + reference_video_name)
                return -1
            if not mod_ret_val:
                print("Can not access video: " + modified_video_name)
                return -1
            ...
            for ref_face_region, mod_face_region in zip(ref_face_regions, mod_face_regions):
                MSSSIM_dist = compute_MSSISM(ref_face_region, mod_face_region)
                BRISQUE_score_ref = cv2.quality_QualityBRISQUE.compute(ref_face_region, BRISQUE_MODEL_FILE,
                                                                       BRISQUE_RANGE_FILE)
                BRISQUE_score_mod = cv2.quality_QualityBRISQUE.compute(mod_face_region, BRISQUE_MODEL_FILE,
                                                                       BRISQUE_RANGE_FILE)
                out_file_BRISQUE.writelines('%d, %.6f,  %.6f\n' % (frame_count, BRISQUE_score_ref, BRISQUE_score_mod))
            ...
            if DEBUG:
                ...
                out_img_filename = "/tmp/processed-" + str(frame_count) + ".jpg"
                ...
                for ref_detected_face in ref_face_regions:
                    ...
                    for ref_face_block in ref_face_blocks:
                        out_img_filename = "/tmp/" + reference_video_name + "-frame_" + str(
                            ...
                            face_count) + "_-_block_" + str(block_count) + ".jpg"
